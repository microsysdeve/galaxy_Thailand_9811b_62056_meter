...
{//注意:建表需要参考E2参数排布对应的文档,下面两个秒对应量仅用于参考
    {JBLEN,             0*E2_PROTECT_LEN},  //0x00 校表参数
    {JBLEN,             0*E2_PROTECT_LEN},  //0x01 校表参数+电量
    {E2_PROTECT_LEN,    2*E2_PROTECT_LEN},  //0x02 参数
};
...
{//对指定no号的E2参数区间进行crc校验判断
    ...
    if(ucNo <= 1 || ucNo >= dim(gCs_E2ParaChkTab))                                   //正常检测时间未到
    {
        return;
    }
...
void SysE2ParaSetManage(uint16 uiAddr, uint8* pucBuf,uint8 ucLen)
{
    ...
    uint8   ucBlkLen;
    uint16  uiE2Addr;
    uint8   ucszChk1[E2_PARAMAX_LEN];
    uint8   ucszChk2[E2_PARAMAX_LEN];
    uint8*  p = ucszChk1;
...
//    }
    ...
    else if(gui_RefreshEvent&flgEtPara_ADKCal_11)
    {
        ADK_AutoChkPW(gs_Channel.ucSta);
        ADK_AutoChkUrms();
        ADK_AutoChkIrms(gs_Channel.ucSta);
...
//        guc_ShowStCom = 10;                                //显示10s
        gui_RefreshEvent&=~flgEtPara_ADKCal_11;
    }
    else if(gui_RefreshEvent&flgEtPara_ADKCal_15)
    {         
        ADK_AutoChkAngle(gs_Channel.ucSta);
//        gul_ShowState |= flgShowSt_Com;                    //通讯显示电话机符号
//        guc_ShowStCom = 10;                                //显示10s
        gui_RefreshEvent&=~flgEtPara_ADKCal_15;
    }
    else if(gui_RefreshEvent&flgEtPara_ADKCal_31)
    {             
        ADK_AutoChkTinyPW(gs_Channel.ucSta);
