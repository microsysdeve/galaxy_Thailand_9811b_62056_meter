...
//    uint32 ulValue;
    MemCpy(s_FrmInfo->uiID.byte,s_FrmInfo->ucbuf,4);    //数据标识
     ...
     if(s_FrmInfo->ucLen == 2)
        {
            guc_DLT1997 = 0x55;
            s_FrmInfo->ucReply = 1;                         //97规约 广播地址也回复
            s_FrmInfo->uiID.word[1] = 0;

            /*if(s_FrmInfo->uiID.lword==0xe410)
            {
                s_FrmInfo->ucbuf[0]=Factbyte;
                s_FrmInfo->ucLen = 1;
                return Ret_OK;
            }
            if(s_FrmInfo->uiID.lword==0xC119)
            {
                s_FrmInfo->ucbuf[0]=Factzero;
                s_FrmInfo->ucLen = 1;
                return Ret_OK;
            }*/
            for(i=0;i<NumofCore97ID;i++)
            {
                if(s_FrmInfo->uiID.lword==gs_CoreTab97[i].uiID)
                {
                    s_FrmInfo->uiID.lword = gs_CoreTab97[i].ulID;
                    s_FrmInfo->ucCmd = 0x11;
                    if(Ret_Err==RWData(s_FrmInfo))
                    {
                        s_FrmInfo->ucLen = 1;                    
                    }
                    s_FrmInfo->uiID.word[1] = 0;
                    s_FrmInfo->uiID.word[0] = gs_CoreTab97[i].uiID;
                    s_FrmInfo->ucCmd = 0x01;
                    return Ret_OK;
                }
            }
            if(i >= NumofCore97ID)
...
uint8 FactoryWR(S_FRAMEINFO *s_FrmInfo)
{
    ...
    if(s_FrmInfo->ucCmd == 0x0F)
    {
        guc_DLT1997 = 0x55;
        if(JudgePassword(s_FrmInfo->ucbuf ) !=0xff)
        {
            SysE2ParaSetManage(s_FrmInfo->uiID.byte[0]*3 +EEP_645PASSWD1, s_FrmInfo->ucbuf + 5, 3);//写入新密码
            *(s_FrmInfo->ucbuf +1) =  *(s_FrmInfo->ucbuf +5 );
            *(s_FrmInfo->ucbuf +2) =  *(s_FrmInfo->ucbuf +6 );
            *(s_FrmInfo->ucbuf +3) =  *(s_FrmInfo->ucbuf +7 );
            s_FrmInfo->ucLen = 4;
            return Ret_OK;
        }
        else
        {
            s_FrmInfo->ucErr.word = ComErrBit_Other;                //密码错误
            s_FrmInfo->ucLen = 0;
            return Ret_OK;
        }           
    }
    else if (s_FrmInfo->ucCmd == 0x12 && s_FrmInfo->ucLen == 0)
    {
        guc_DjCheckFlg|=flgInsDjEvent;
        s_FrmInfo->ucLen = 0;
        return Ret_OK;
    }
    else  if(s_FrmInfo->ucCmd == 0x04)
    {
        guc_DLT1997 = 0x55;
        s_FrmInfo->uiID.word[1] = 0;
        if(s_FrmInfo->uiID.lword == 0xC119)
        {
            s_FrmInfo->ucLevel = s_FrmInfo->ucbuf[2];
            if(s_FrmInfo->ucLevel != 0x00)                           //判断权限
            {
            ...
            if(s_FrmInfo->ucbuf[3] != 0x11 ||s_FrmInfo->ucbuf[4] != 0x11 ||s_FrmInfo->ucbuf[5] != 0x11)
            {
                s_FrmInfo->ucAtb = Const_Atb_Pswd_Err;
                s_FrmInfo->ucErr.word = ComErrBit_ErrPw;             //密码错误
                s_FrmInfo->ucLen = 1;
            ...
            if((guc_FactoryType!=FactorMd) && ((*(gs_EnergyA.lCP)+ Eny_GetEp1(0,0))>10000) )
            {
                s_FrmInfo->ucAtb = Const_Atb_Pswd_Err;
                s_FrmInfo->ucErr.word = ComErrBit_Other;             //密码错误
                s_FrmInfo->ucLen = 1;
        ...
        s_FrmInfo->ucLevel = s_FrmInfo->ucbuf[2];
        if( s_FrmInfo->ucLevel != 0x01)                           //判断权限
        {
            s_FrmInfo->ucAtb = Const_Atb_Pswd_Err;
            s_FrmInfo->ucErr.word = ComErrBit_ErrPw;             //密码错误
            s_FrmInfo->ucLen = 1;
        ...
        //厂内状态,数据指针指向未处理数据
        MemCpy(s_FrmInfo->ucbuf,s_FrmInfo->ucbuf+6,s_FrmInfo->ucLen-6);
    ...
    else
    {
        ...
        if(s_FrmInfo->uiID.lword==0x2055FF00)     //初始化参数
        {
        ...
        if( (guc_FactoryType!=FactorMd) || ((*(gs_EnergyA.lCP)+ Eny_GetEp1(0,0))>1000))
        {
            s_FrmInfo->ucErr.word = ComErrBit_ErrPw;                //没有找到数据标识
            s_FrmInfo->ucLen = 1;
            return Ret_OK;
        ...
        //     if(s_FrmInfo->uiID.lword==0x4F520000)     //自动校表 100%Ib
        //    {   
        ...
        //        gui_RefreshEvent|=flgEtPara_Init;
        //    }
        if(s_FrmInfo->uiID.lword==0x20000001)  //满度初始化
        {
            if(s_FrmInfo->ucLen == 0x23)
            {
                ...
                FCpyTMem((uint8*)&gs_JbPm.ui_Un,InitPara0+2,4);             //拷贝Flash数据到RAM中
                gs_JbPm.ul_Gatep=Uint8_To_Uint32(s_FrmInfo->ucbuf+30);      //有功门限
                gs_JbPm.ul_Gateq = 0x1520AD43;//0x1B9C283E;   矩华参数230 10（60） 1600
                gs_JbPm.ul_GateOSC = 0x4EAC8F6;//0x66d00ce;
            ...
            {
                MemCpy((uint8 *)&gs_JbPm.ui_MeterC, s_FrmInfo->ucbuf+4, 22);//电表常数-潜动门限
                MemSet((uint8 *)&gs_JbPm.ul_SCP, 0, 30); //有功比差-同道2角差
                MemCpy((uint8 *)&gs_JbPm.ul_PG, s_FrmInfo->ucbuf+26, 20);//有功显示比例系数-通道2显示比例系数
                MemSet((uint8 *)&gs_JbPm.ul_SCI1_800, 0, 8); //有效值比差
                MemCpy((uint8 *)&gs_JbPm.uc_AnaG, s_FrmInfo->ucbuf+46, 1);  //模拟增益 
            }
        ...
        else if(s_FrmInfo->uiID.lword==0x200000A3)                          //B通道功率比差值
        {
            gs_JbPm.ul_SCQ=Uint8_To_Uint32(s_FrmInfo->ucbuf+4);             //有功比差值
            gs_JbPm.ui_JbCRC=do_CRC((uint8*)&gs_JbPm,sizeof(S_JBPM)-2);     // 校表参数的CRC结果
            //SysE2ParaSetManage(EEP_JBTOTAL,(uint8*)&gs_JbPm,sizeof(S_JBPM));         //写入E2中 
            BE_WriteP(EEP_JBTOTAL, (uint8*)&gs_JbPm, sizeof(S_JBPM));
            BE_WriteP(EEP_JBTOTAL+EEP_BAK_LEN, (uint8*)&gs_JbPm, sizeof(S_JBPM));
        ...
        else if(s_FrmInfo->uiID.lword==0x200000A6)                          //A角差
        {
            gs_JbPm.uc_PhcCtrl2=*(s_FrmInfo->ucbuf+4);                      //有功比差值
            gs_JbPm.ui_JbCRC=do_CRC((uint8*)&gs_JbPm,sizeof(S_JBPM)-2);     // 校表参数的CRC结果
            //SysE2ParaSetManage(EEP_JBTOTAL,(uint8*)&gs_JbPm,sizeof(S_JBPM));    //写入E2中 
            BE_WriteP(EEP_JBTOTAL, (uint8*)&gs_JbPm, sizeof(S_JBPM));
            BE_WriteP(EEP_JBTOTAL+EEP_BAK_LEN, (uint8*)&gs_JbPm, sizeof(S_JBPM));
        ...
        else if(s_FrmInfo->uiID.lword==0x200000A7)                          //二次补偿值
        {
            gs_JbPm.ul_PARAQC=Uint8_To_Uint32(s_FrmInfo->ucbuf+4);          //有功比差值
            gs_JbPm.ui_JbCRC=do_CRC((uint8*)&gs_JbPm,sizeof(S_JBPM)-2);     // 校表参数的CRC结果
            SysE2ParaSetManage(EEP_JBTOTAL,(uint8*)&gs_JbPm,sizeof(S_JBPM)); //写入E2中 
            //        BE_WriteP(EEP_JBTOTAL, (uint8*)&gs_JbPm, sizeof(S_JBPM));
            //        BE_WriteP(EEP_JBTOTAL+EEP_BAK_LEN, (uint8*)&gs_JbPm, sizeof(S_JBPM));
...
uint8 ClearMeter(S_FRAMEINFO *s_FrmInfo)
{
    ...
    if((guc_FactoryType!=FactorMd) && ((*(gs_EnergyA.lCP)+ Eny_GetEp1(0,0))>10000) )
    {
        ...
        if( (s_FrmInfo->ucLevel != 0x02))                 //没有编程键、密码不对
        {
            s_FrmInfo->ucErr.word = ComErrBit_ErrPw;        //权限不足返回
            s_FrmInfo->ucLen = 1;
            return Ret_OK;
...
uint8 ReadCurEgyN(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
   /*  int16 Eadd; */
/*     uint32 *pEnyData; */
/*     //Word32 data1; */
...
/*     pEnyData=(uint32*)gs_EnergyAN.lCP;                    //获取电量数据结构首地址 */


/*     if(s_FrmInfo->ucAtb & Const_Atb_DataJC)     //如果是集抄的话 */
/*     { */
/*        if(s_FrmInfo->uiID.byte[2]==3) */
/*         { */
/*              pEnyData=(uint32*)gs_EnergyAN.ulP;  */
/*             data1=*(pEnyData); */

/*         } */
/*          else */
/*         { */
/*              pEnyData=(uint32*)gs_EnergyAN.ulN;  */
/*             data1=*(pEnyData); */
/*         } */
/*         for(uint8 i = 0; i < 1; i++) */
/*         { */
...
/*             data1=*(pEnyData); */
/*         } */
/*         else if(s_FrmInfo->uiID.byte[2]==3) */
/*         { */
/*              pEnyData=(uint32*)gs_EnergyAN.ulP;  */
/*             data1=*(pEnyData); */
/*             //data1 = 0X1345; */
/*         } */
/*          else if(s_FrmInfo->uiID.byte[2]==4) */
/*         { */
/*              pEnyData=(uint32*)gs_EnergyAN.ulN;  */
/*             data1=*(pEnyData); */
/*              //data1 = 0X2468; */
...
/*         Eadd = Eny_GetEpN(s_FrmInfo->uiID.byte[2]); */
/*         data1 += Eadd; */
...
uint8 ReadCurEgy(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    if(s_FrmInfo->ucAtb & Const_Atb_DataJC)     //如果是集抄的话
    {
        ...
        {
            flag=0;
            data1=*(pEnyData+s_FrmInfo->uiID.byte[2]+i);
    ...
    {                                           //取得某个费率的电量增量
        flag=0;
        guc_ShowEnergyFlag=0;
        data1=*(pEnyData+s_FrmInfo->uiID.byte[2]+s_FrmInfo->uiID.byte[1]);
        Eadd = Eny_GetEp1(s_FrmInfo->uiID.byte[2], s_FrmInfo->uiID.byte[1]);
        data1 += Eadd;
        if( s_FrmInfo->uiID.byte[1] != 0x00) //费率电量为0
        {
          data1 =0x00;
        }
        if(s_FrmInfo->uiID.byte[2]==0)                      //组合
...
//extern const uint8 code InitPara0[];
...
uint8 ComRWE2Para(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    if(s_FrmInfo->ucCmd == 0x11)                //如果是读数据的话
    {
        if(s_FrmInfo->uiID.lword==0x04000705)   //通讯口3通讯速率特征字
        {
         ...
         if(s_FrmInfo->uiID.lword==0x04000602)   //通讯口3通讯速率特征字
        {
            s_FrmInfo->ucbuf[0]=0x55;
            s_FrmInfo->ucLen =1;                //回复的数据长度
            return Ret_OK;
        }
          if(s_FrmInfo->uiID.lword==0x04000603)   //通讯口3通讯速率特征字
        {
            s_FrmInfo->ucbuf[0]=0x55;
            s_FrmInfo->ucLen =1;                //回复的数据长度
            return Ret_OK;
        ...
        {
        	  FCpyTMem(buf,&InitPara0[44],6);
            s_FrmInfo->ucbuf=buf;
            s_FrmInfo->ucLen =6;                //回复的数据长度
            return Ret_OK;
        } 
        if(s_FrmInfo->uiID.lword==0x04000405)   //额定电流
        {
        	  FCpyTMem(buf,&InitPara0[50],6);
            s_FrmInfo->ucbuf=buf;
            s_FrmInfo->ucLen =6;                //回复的数据长度
            return Ret_OK;
        } 
        if(s_FrmInfo->uiID.lword==0x04000406)   //最大电流
        {
        	  FCpyTMem(buf,&InitPara0[56],6);
            s_FrmInfo->ucbuf=buf;
            s_FrmInfo->ucLen =6;                //回复的数据长度
            return Ret_OK;
        } 
...
//    BE_ReadP(CTable->pAddr,s_FrmInfo->ucbuf,2);
    //2个字节变成3个字节的BCD
    temp.word[0]=Uint8_To_Uint16(s_FrmInfo->ucbuf);
    temp.lword=Hex2BCD(temp.lword);
    //temp.word[0]=IntHex2BCD(temp.word[0]);
...
/*                 //s_FrmInfo->ucLen = 20; */
/*                 MemCpy(s_FrmInfo->ucbuf,XDATA_RAMZONE+25,4); */
/*                 s_FrmInfo->ucLen = 4; */
/*             }else */
/*             { */
/*                 //MemCpy(s_FrmInfo->ucbuf,XDATA_RAMZONE+25,4); */
...
/*             } */
/*         } */
/*         else if(s_FrmInfo->uiID.byte[1] == 0x03)           //无功1 */
/*         { */
/*            //  if(ucDjMode&BIT0) */
/*             { */
/*                 //MemCpy(s_FrmInfo->ucbuf,XDATA_RAMZONE+5,20); */
...
/*          //   } */
/*         } */
/*         else if(s_FrmInfo->uiID.byte[1] == 0x04)           //无功2 */
/*         { */
/*                MemCpy(s_FrmInfo->ucbuf,XDATA_RAMZONE+25,4); */
/*                 s_FrmInfo->ucLen = 4; */
/*         } */
...
uint8 ReadVar(S_FRAMEINFO *s_FrmInfo, S_CORETAB code * CTable)
{
    ...
    if(pVar->Atb==0x01)
    {
        ...
        if(pVar->ulAdrr == DATAP)               //火线功率
        {
          if(SETA == gs_Channel.ucSta)
          {
            ulVarValue=CalRMS(DATAP);
          }else if(SETB == gs_Channel.ucSta)       //零线功率
          {
