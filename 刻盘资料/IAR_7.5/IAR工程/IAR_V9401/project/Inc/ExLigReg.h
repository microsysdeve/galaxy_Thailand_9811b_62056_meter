...
__sfr __no_init volatile union
{
  ...
  struct 
  {
    ...
    unsigned char PMG :1;
    unsigned char No_Data1 :1;
    unsigned char No_Data2 :1;
...
//  unsigned char RTCSFTO;
  struct 
  {
    ...
    unsigned char No_Data1: 1;
    unsigned char No_Data2 : 1;
    unsigned char No_Data3 : 1;
    unsigned char No_Data4 : 1;
    unsigned char No_Data5 : 1;
    unsigned char TERROR : 1;
  } RTCSFTO;
} @ 0x90;

#define TSRC   RTCSFTO.TSRC;
#define TTYPE  RTCSFTO.TTYPE;
#define TERROR RTCSFTO.TERROR;

//全局变量
extern unsigned long  u32PMdatal;//计量寄存器数据低32位，用于计量寄存器数据读写存放
extern unsigned char  u8PMdatah;//计量寄存器数据高8位，用于计量寄存器数据读写存放
