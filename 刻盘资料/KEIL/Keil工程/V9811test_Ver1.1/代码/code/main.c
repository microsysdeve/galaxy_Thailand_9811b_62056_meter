...
/***************************** Copyright by Vango ***********************************
	...
	文件类型：V9811TEST C51 Keil3编写编译
	文件作用：主要用于SuperDebug下载测试,下载完成后，若看见LED灯连续以500ms闪烁3次，停顿2S
			  时间后以300ms间隔连续闪烁5次，再次停顿1.8S时间后以200ms间隔连续闪烁7次。再停顿1.7S则
			  进入新的循环。则FLASH下载成功。SuperDebug可成功下载至V9801的Flash中去。

	创 建 人：DL
	创建日期：2012年4月20日
	当前版本：V9811Test_Ver1.1
	
	版本信息：V9811Test_ver1.0
			  V9811Test_ver1.1  
**************************************************************************************/
#include "delay.h"
#include "V98xx.h"
...
void main()
{
	...
	{
		Test1(3);
		delayms(500);//由于要加上Test1灭灯结束的后的延时，所以延时为1.5S+闪灯间隔时间=2S
		delayms(500);
		delayms(500);
