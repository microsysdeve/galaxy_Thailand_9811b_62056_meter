...
uint16 GetCRCData(void)
{
  uint16 u16Ret;
  uint8 code *point;
  point = (uint8 code *)0x7fff;
  u16Ret = *point;
  u16Ret = u16Ret<<8;
   point = (uint8 code *)0x7ffe;
  u16Ret += *point; 
  return u16Ret;
}
...
uint32 GetCRCLen(void)
{
  uint32 u32Ret;
  uint8 code *point;
  point = (uint8 code *)0x7ffd;
  u32Ret = *point;
  u32Ret = u32Ret<<8;
   point = (uint8 code *)0x7ffc;
  u32Ret += *point; 
  u32Ret = u32Ret<<8;
   point = (uint8 code *)0x7ffb;
  u32Ret += *point; 
  u32Ret = u32Ret<<8;
   point = (uint8 code *)0x7ffa;
  u32Ret += *point; 
  return u32Ret;
...
uint8 checkCRC(void)
{
    ...
    else if(u32CrcLen<0x8000)
    {
      u16CalCRCData = do_CRC_root(0x5555,0,0x400);
      u16CalCRCData = do_CRC_root(u16CalCRCData,0x401,u32CrcLen-0x401);
    }
	else
	{
	  return 0;
