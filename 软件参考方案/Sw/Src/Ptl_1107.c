...
{   //obsi              uint             param     atb           pfindex  
    ...
    {"96.7.1",          ")",            0x00,       IEC_RO,     0x04},    //掉电总次数
    {"96.77.1*1",       ")",            0x00,       IEC_RO,     0x04},    //上1次掉电时间和结束时间
    {"96.77.1*2",       ")",            0x10,       IEC_RO,     0x04},    //上2次掉电时间和结束时间
    {"96.77.1*3",       ")",            0x20,       IEC_RO,     0x04},    //上3次掉电时间和结束时间
    {"96.77.1*4",       ")",            0x30,       IEC_RO,     0x04},    //上4次掉电时间和结束时间
    {"96.77.1*5",       ")",            0x40,       IEC_RO,     0x04},    //上5次掉电时间和结束时间
    {"96.77.1*6",       ")",            0x50,       IEC_RO,     0x04},    //上6次掉电时间和结束时间
    {"96.77.1*7",       ")",            0x60,       IEC_RO,     0x04},    //上7次掉电时间和结束时间
    {"96.77.1*8",       ")",            0x70,       IEC_RO,     0x04},    //上8次掉电时间和结束时间
    {"96.77.1*9",       ")",            0x80,       IEC_RO,     0x04},    //上9次掉电时间和结束时间
    {"96.77.1*10",      ")",            0x90,       IEC_RO,     0x04},    //上10次掉电时间和结束时间
    {"96.77.1*11",      ")",            0xA0,       IEC_RO,     0x04},    //上11次掉电时间和结束时间
    {"96.77.1*12",      ")",            0xB0,       IEC_RO,     0x04},    //上12次掉电时间和结束时间
...
//      gs_PtlCrt.uc_TimeOut=COMOUTTM;
    ...
    case '1':                            //进入编程模式
        ...
        {
            Init_Uart4(3, 0);        // 初始化模拟串口,由于这个是波特率固定的所以不需要切换波特率
        }
        else
        {
            Init_Uart2(3, 0);        // 初始化模拟串口,由于这个是波特率固定的所以不需要切换波特率
        }
        DelayXms(100);                                                 //睡眠20ms
