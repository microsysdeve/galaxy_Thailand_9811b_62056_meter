...
//    {
//        BE_ReadP(EEP_COMADD, s_FrmInfo->ucbuf, 6);          //读取表地址
...
uint8 WriteMeterAddr(S_FRAMEINFO *s_FrmInfo)
{
    ...
    {
        s_FrmInfo->ucErr |= ComErrBit_ErrPw;            //权限不足
        s_FrmInfo->ucLen = 1;
    }
    return Ret_OK;
}
...
uint8 RWData(S_FRAMEINFO *s_FrmInfo)
{
    ...
    {
        s_FrmInfo->ucErr = ComErrBit_Nodata;            //没有找到数据标识
        s_FrmInfo->ucLen = 1;
        return Ret_OK;
    }
...
uint8 FactoryRD(S_FRAMEINFO *s_FrmInfo)
{
    ...
    if(s_FrmInfo->uiID.lword==0x20000002)
    {
        BE_ReadP(EEP_JBTOTAL,s_FrmInfo->ucbuf + 4,sizeof(S_JBPM));
        s_FrmInfo->ucLen = 4+sizeof(S_JBPM);
        return Ret_OK;
    }
    return Ret_Err;
}
...
//    uint8 i;
    ...
    {
        if((gui_SystemState & flgStSys_Program)
            ||(guc_FactoryType==FactorMd))              //编程键下并且开表盖状态，可以进入厂内状态
        {
            guc_DyProDmOut = 30;
        ...
        {
            s_FrmInfo->ucErr = ComErrBit_ErrPw;            //没有找到数据标识
            s_FrmInfo->ucLen = 1;
            return Ret_OK;
        }
    }
    ...
    {
        EnyB_AutoCheck((uint8 *)(s_FrmInfo->ucbuf + 4));
    }
    else if(s_FrmInfo->uiID.lword==0x4F520001)     //自动校表  100%Ib 0.5L 默认PHCCtrl1已写0
    {
//      EnyB_AutoChkAngle((uint8 *)(s_FrmInfo->ucbuf + 4));
    }
    ...
    else if(s_FrmInfo->uiID.lword==0x20000001)  //满度初始化
    {
        ...
        //00 3F E4 D6       //功率显示系数
        //3D 1E 7F FF       //电压显示系数
        //10 FC 14 7A       //I1电流显示系数
        //0E 27 66 66       //I2电流显示系数
        //03                //模拟增益配置
        MemCpy((uint8*)&gs_JbPm.ui_MeterC,s_FrmInfo->ucbuf+4,15);       //常数,额定电压，额定电流、有功门限制，潜动门限制
        MemCpy((uint8*)&gs_JbPm.ul_PG,s_FrmInfo->ucbuf+19,17);          //功率、电压、电流、零线电流显示系数和增益。
        MemSet((uint8*)&gs_JbPm.ul_SCP,0,30);                           //比差、二次补偿、角差清零
...
//      gui_RefreshEvent |= flgEtPara_EnyBottom;
//  }
    else if(s_FrmInfo->uiID.lword==0x20000002)
    {
        SysE2ParaSetManage(EEP_JBTOTAL, s_FrmInfo->ucbuf+4,sizeof(S_JBPM));
        gui_RefreshEvent |= flgEtPara_EnyBottom;
    }
    s_FrmInfo->ucLen = 0;
    return Ret_OK;
...
//  if(pVar->Atb==0x01)
//  {
        ulVarValue=EnyB_CalRMS(pVar->ulAdrr);       //电压，电流,零线电流，功率，功率因数
