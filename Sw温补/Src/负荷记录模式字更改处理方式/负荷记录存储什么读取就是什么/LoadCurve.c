...
uint8 BuildTotalCurveFrame(S_CURVETOTAL *s_TotalCurve,uint8 *buff)
{
    ...
    ucTotalType=s_TotalCurve->ucType;
    if(ucTotalType == 0)
    {
        return 0;
    }
...
{    
//  S_CURTIME s_StartLoadCurve;     //当前时钟数据
//    uint8 uc_LoadCurveMode;
//    uint32 ulPt=0;

//    MemCpy((uint8*)&s_StartLoadCurve,(uint8*)&gs_StartLoadCurve,5);    //写入时间
...
//  if((guc_LCTimeFlag == 0x55                                     //起始时间改变
//     && CmpTm((uint8 const *)&s_StartLoadCurve, (uint8 const *)&gs_StartLoadCurve,4) != CMP_EQU)
//     || (guc_LCTimeFlag == 0xAA && uc_LoadCurveMode != guc_LoadCurveMode))//模式字改变
//  {
//      uint8 i;
//      uint8 *buff;
//      for(i=0;i<5;i++)
//      {
//          MemSet(buff,0x00,gs_CurveStoreTab[i].ucLen);
//          BE_WriteP(gs_CurveStoreTab[i].uiCntAddr, (uint8*)&ulPt, 3);   //清负荷记录
//          BE_WriteP(gs_CurveStoreTab[i].uiPtAddr, (uint8*)&ulPt, 2);    //清负荷记录
//          BE_WriteP(gs_CurveStoreTab[i].uiBaseAddr, buff, gs_CurveStoreTab[i].ucLen);    //清负荷记录
//      }
//      //gui_RefreshEvent |= flgEtPara_Fee;
//      //guc_LCTimeFlag = 0;
//  }
    
    if((guc_LCTimeFlag&0x0f) == 0x05)
    {
