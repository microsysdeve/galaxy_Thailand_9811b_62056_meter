...
//  uc_QjAddress += BE_Const_ID;                    // 确定器件地址
    ...
    for (;;)
    {
        ...
        if (uc_retrycnt >= 3)                       // 重试达到3次
        {
            ...
            gs_MeterState.ui_MeterState1.RunCode1Word.EepErr=true;
            Rpt_StaSet(RPT_EEP);
            Rpt_StaSet(RPT_EEPINIT);
            return 2;                               // 写入失败
        }
...
//#endif
    ...
    gs_MeterState.ui_MeterState1.RunCode1Word.EepErr=false;
    Rpt_StaFlgClr(RPT_EEP);
    Rpt_StaFlgClr(RPT_EEPINIT);
    return 0;                                       // 正确返回
}
/*=========================================================================================\n
