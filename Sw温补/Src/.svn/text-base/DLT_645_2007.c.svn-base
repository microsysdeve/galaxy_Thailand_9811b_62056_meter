...
uint8 RWData(S_FRAMEINFO *s_FrmInfo)
{
    uint8 Index,i;
        ...
        if(s_FrmInfo->ucCmd == 0x14)
        {
            ...
            else if(s_FrmInfo->uiID.lword==0x0400040f)                 //电表位置信息
            {
                ApiReverseData(s_FrmInfo->ucbuf,4);                     //取反操作
                ApiReverseData(s_FrmInfo->ucbuf+4,4);                   //取反操作
                ApiReverseData(s_FrmInfo->ucbuf+8,3);                   //取反操作
            }
        }
        ...
        {          
            ...
            if(((s_FrmInfo->uiID.lword&0xffffff00) == 0x03300500 && s_FrmInfo->uiID.byte[0] != 0)       //时段表编程
               ...
               || ((s_FrmInfo->uiID.lword&0xffff0000) == 0x06100000))   //负荷记录相关
            {
...
//        }
    ...
    else if(s_FrmInfo->uiID.lword==0x20000040)    //电池校正
    {
        uint16 uitemp;
        uitemp = Mcu_BatCal();
        MemCpy(s_FrmInfo->ucbuf+4, (uint8*)&uitemp, 2);
        MemCpy(s_FrmInfo->ucbuf+6, (uint8*)&gi_BatCal, 2);
...
uint8 ReadCurEgy(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    if(s_FrmInfo->ucCmd == 0x14)
    {
        ...
        uint32 buf;
        MemCpy((uint8*)&buf,s_FrmInfo->ucbuf,4);
        buf = BCD2Hex(buf);
        buf = buf * 10;
        MemCpy((uint8*)(pEnyData+s_FrmInfo->uiID.byte[2]*5+s_FrmInfo->uiID.byte[1]),(uint8*)&buf,4);  
...
uint8 ComRWE2Para(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    if(s_FrmInfo->ucCmd == 0x11)                //如果是读数据的话
    {
        ...
        if(s_FrmInfo->uiID.lword==0x04000705)   //通讯口3通讯速率特征字
        {
            s_FrmInfo->ucbuf[0]=0x08;
            s_FrmInfo->ucLen =1;                //回复的数据长度
            return Ret_OK;
        }
...
uint8 Report(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    uint8 tmpbuf;   
    //复位主动上报状态字
    if(s_FrmInfo->ucCmd == 0x14 && s_FrmInfo->uiID.lword == 0x04001503)
    {   //红外通道要求认证通过
//        if((s_FrmInfo->ucPort==ComIndex_Uart2)
...
//        }
        if(guc_RptClrCnt != 0)
        {
            REPORTSTATE s_RptSta;
            uint16 *pRptB, *pRptSta, *pRpt;
            
            MemCpy((uint8*)&s_RptSta, s_FrmInfo->ucbuf, RPTSTALEN);
            
            pRpt  = (uint16 *)&gs_ReportState;
            pRptB  = (uint16 *)&gs_RptStaB;
            pRptSta = (uint16 *)&s_RptSta;
            for(uint8 i=0; i<6; i++)
            {
              for(uint8 j=0; j<15; j++)
              {
                if((pRpt[i] & (uint16)BIT32MAP[j]) != 0) //主动上报状态字发生的位
                {
                  if(((pRptSta[i] & (uint16)BIT32MAP[j]) == 0) && ((pRptB[i] & (uint16)BIT32MAP[j]) != 0))
                  {
                    s_FrmInfo->ucErr.word=ComErrBit_Other;
                    s_FrmInfo->ucLen = 1;  
                    return Ret_OK;
...
//              pRptSta[i] |= pRpt[i]; //根据主动上报状态字明位或者暗位更新复位字
            }
            Rpt_UpdateSta((uint16*)&s_RptSta);
            s_FrmInfo->ucLen = 0;
        }
        else
        {
    ...
    else if(s_FrmInfo->ucCmd == 0x11 || s_FrmInfo->ucCmd == 0x12)        //指令读
    {
        ...
        if(s_FrmInfo->ucCmd == 0x12)
        {
            ...
            tmpbuf = s_FrmInfo->ucbuf[0]; //后续帧标志
            //s_FrmInfo->ucbuf[10]=  s_FrmInfo->ucbuf[0];            
            //s_FrmInfo->ucLen = 11;  
        }
        
        MemCpy((uint8*)&gs_RptStaB, (uint8*)&gs_ReportState, RPTSTALEN);    //保存当前状态
        uint16 *pRpt, *pRptB;
        pRpt  = (uint16 *)&gs_ReportState;
        pRptB = (uint16 *)&gs_RptStaB;
        for(i=0; i<6; i++, pRpt++, pRptB++)
        {
            *pRptB = ~(*pRpt);
        }
        MemCpy(s_FrmInfo->ucbuf, (uint8*)&gs_ReportState, RPTSTALEN);       //
        s_FrmInfo->ucbuf[12] = 0xAA;
        uint8 *pCnt;
        uint8 ucCnt=0;
//        pCnt = (uint8 *)&gs_RptCnt.MYupdate;
//        for(i=RPTCNTLEN; i>0; i--,pCnt--)
//        {
//            if(*pCnt != 0)
//            {
//                ucCnt++;
//                s_FrmInfo->ucbuf[12+ucCnt] = *pCnt;   
//            }
//        }
        pCnt = (uint8 *)&gs_RptCnt.LoopErr;
        ...
        s_FrmInfo->ucbuf[13+ucCnt] = 0xAA;
        s_FrmInfo->ucbuf[14+ucCnt] = tmpbuf;
   
        s_FrmInfo->ucLen = 14 + ucCnt + (s_FrmInfo->ucCmd - 0x11);
        
        //guc_RptClrCnt = 30;   //清零主动上报状态字计时
        SysE2ReadData(EEP_RPTCLRTM, (uint8*)&guc_RptClrCnt, 1);
    }else
    {
        s_FrmInfo->ucErr.word=ComErrBit_Other;
        s_FrmInfo->ucLen = 1;
...
* @修改内容: 
===========================================================================================*/
uint8 Ldc_ReadType(S_FRAMEINFO *s_FrmInfo)
{
    uint8 ucType;
//无需读总负荷记录
    if(s_FrmInfo->uiID.byte[1] == 0x01 || s_FrmInfo->uiID.byte[1] == 0x02)    //电压 电流
    {
        ucType = ((s_FrmInfo->uiID.byte[1]<<4)|0x01);
    }
    else if(s_FrmInfo->uiID.byte[1] == 0x03)    //有功功率
    {
        ucType = 0x02;
    }
    else if(s_FrmInfo->uiID.byte[1] == 0x05)    //功率因数
    {
        ucType = 0x03;
    }
    else if(s_FrmInfo->uiID.byte[1] == 0x06)    //有功总电能
    {
        ucType = (s_FrmInfo->uiID.byte[0]<<4)|0x04;
    }
...
uint8 ReadCurveJl(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    else if(s_FrmInfo->ucCmd == 0x12)                                           //读后续帧
    {
        s_FrmInfo->ucLen = 0;
        ucFrameNum       = s_FrmInfo->ucbuf[0];                                 //保存帧序号
        if(gs_LdcFrame.SeqFrameEnd == true)                             //尾帧，正常情况已完
        {                                                               //除非读主动上报状态字
...
//                }
                ...
                s_FrmInfo->ucbuf[13+ucCnt] = 0xAA;
                s_FrmInfo->ucbuf[14+ucCnt] = ucFrameNum;
    ...
    else if(s_FrmInfo->ucCmd == 0x11)                                           //读首帧
    {
        if(s_FrmInfo->ucLen < 10
           || JudgeIsNum(s_FrmInfo->ucbuf,6)==Const_Data_Err)
        {
            s_FrmInfo->ucErr.word = ComErrBit_Other;
            s_FrmInfo->ucLen      = 1;
            return Ret_OK;        
        ...
        gs_LdcFrame.TotalCnt = APIByteBCD2HEX(s_FrmInfo->ucbuf[0]);             //负荷点数
        if(gs_LdcFrame.TotalCnt == 0 || gs_LdcFrame.TotalCnt > 96)
        {            
            s_FrmInfo->ucErr.word = ComErrBit_Other;
            s_FrmInfo->ucLen      = 1;
            return Ret_OK; 
        }
        MemSet((uint8*)&gs_LdcPos, 0x00, sizeof(S_LDCPT));
        MemCpy((uint8 *)&gs_LdcFrame.tmOG, s_FrmInfo->ucbuf+1, 5);
        MemCpy((uint8 *)&gs_LdcFrame.tmBG, s_FrmInfo->ucbuf+1, 5);              //读取最早时标
        MemCpy((uint8 *)&gs_LdcFrame.tmED, s_FrmInfo->ucbuf+1, 5);
        gs_LdcFrame.SeqFrameEnd = false;                                        //完整帧
        //gs_LdcFrame.ulLastMin   = Ldc_TmMinCal((uint8 *)&gs_LdcFrame.tmBG);
        gs_LdcFrame.ucType      = Ldc_ReadType(s_FrmInfo);                      //负荷类型
        uiGapPt                 = (uint16 *)&gs_LdcSet.uiFH1;                   //负荷记录间隔时间
        gs_LdcFrame.uiGap       = uiGapPt[(gs_LdcFrame.ucType&0x0f) - 1];       //负荷间隔
        Ldc_TimeAdd((uint8*)&gs_LdcFrame.tmED, 
                    gs_LdcFrame.uiGap * (gs_LdcFrame.TotalCnt - 1));              //读取最后时标
        if(Ldc_ZoneChk((uint8 const *)&gs_LdcFrame.tmBG, 
                       (uint8 const *)&gs_LdcFrame.tmED, 
                       gs_LdcFrame.ucType&0x0f) != 0)
        {
            s_FrmInfo->ucLen        = 0;
        ...
        gs_LdcFrame.ulLastMin   = Ldc_TmMinCal((uint8 *)&gs_LdcFrame.tmBG);
        //组帧 第一点数据
        s_FrmInfo->ucLen = Ldc_Get1stRecord(gs_LdcFrame.ucType,
                                            (uint8 *)&gs_LdcFrame.tmBG,
                                            //(uint8 *)&gs_LdcFrame.tmED,
                                            s_FrmInfo->ucbuf);        
        gs_LdcFrame.CurCnt = 1;
    }

    for(;gs_LdcFrame.CurCnt < gs_LdcFrame.TotalCnt;)
    {        
        ucLen = Ldc_GetSeqRecord(gs_LdcFrame.ucType, 
                                 s_FrmInfo->ucbuf+s_FrmInfo->ucLen);
//        if((s_FrmInfo->ucLen+ucLen)>200)
//        {
//            gs_LdcFrame.SeqFrameEnd=false;
//            break;
//        }
        s_FrmInfo->ucLen += ucLen;
        if(gs_LdcFrame.SeqFrameEnd==true)
        {
            break;
        }
        gs_LdcFrame.CurCnt++;
        if(gs_LdcFrame.CurCnt%24 == 0)    //分4帧传
        {
            gs_LdcFrame.SeqFrameEnd = false;
            break; 
        }
    ...
    if(gs_LdcFrame.CurCnt >= gs_LdcFrame.TotalCnt)    //传完
    {
        if(s_FrmInfo->ucCmd == 0x12)
        {
...
uint8 RwTestTime(S_FRAMEINFO *s_FrmInfo, S_CORETAB code* CTable)
{
    ...
    if(s_FrmInfo->ucCmd == 0x11)                        //读数据
    {
        if(s_FrmInfo->uiID.lword == 0x0400010D)         //日期+时间
        {
            BE_ReadP(EEP_TEST_TIME, s_FrmInfo->ucbuf, 7);
//            GetSysClock(s_FrmInfo->ucbuf, Const_hhmmssYYMMDDWW);
            s_FrmInfo->ucLen = CTable->ucLen;
        }
        else
...
//            }
            ...
            {
//                SetSysClock(s_FrmInfo->ucbuf, Const_hhmmssYYMMDDWW);
                MemCpy( buf, s_FrmInfo->ucbuf, 7);
                CRC.word=do_CRC(buf,7);
                buf[7]=CRC.byte[0];
                buf[8]=CRC.byte[1];   //拷贝CRC到RAM中
                BE_WriteP(EEP_TEST_TIME, buf, 9);
                s_FrmInfo->ucLen = 0;
...
void RecoverTestTime(void)
{
    ...
    BE_ReadP(EEP_TEST_TIME,(uint8*)XDATA_RAMZONE,9);
    ...
    if(CRC==do_CRC(XDATA_RAMZONE, 7))      //掉电保存数据正确
    {
        ...
        DelayOSC(5);
        //写RTC时间
        RTCYC   = XDATA_RAMZONE[6];
        RTCMoC  = XDATA_RAMZONE[5];
        RTCDC   = XDATA_RAMZONE[4];
        RTCWC   = XDATA_RAMZONE[3];        
        RTCHC   = XDATA_RAMZONE[2];
        RTCMiC  = XDATA_RAMZONE[1];
        RTCSC   = XDATA_RAMZONE[0];
        DelayOSC(5);
        //RTC禁止写  
        RTCPEN = 0x96;
        RTCPWD = 0x56;
