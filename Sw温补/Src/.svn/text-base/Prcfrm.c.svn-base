    ...
    {                                               //如果为读写的话
        ...
        if(sFrm->ucCmd == 0x14)                     //如果是写数据的话
        {
            ...
            if(guc_FactoryType != FactorMd                          //厂内状态没开的话需要判断密码
               && sFrm->uiID.lword != 0x04001503)                   //第三类数据 状态字更新
            {
        ...
        if(buff[0] == 0x02)                         //02级密码错误
        {
            if(gs_PassWordCtr.ucSt02 != BSST)
            {
                ...
                if(gs_PassWordCtr.ucPW2ErrCnt>=ucBsCnt)         //判断到了闭锁次数
                {
                    ...
                    BE_WriteP(EEP_PW2BSTM,(uint8*)&gs_PassWordCtr.uiDyPW2Out,0x02); //写入闭锁时间
                }
            }
        }
        else if(buff[0] == 0x04)
        {
            if(gs_PassWordCtr.ucSt04!=BSST)
            {
                ...
                if(gs_PassWordCtr.ucPW4ErrCnt>=ucBsCnt)         //判断到了闭锁次数
                {
                    ...
                    BE_WriteP(EEP_PW4BSTM,(uint8*)&gs_PassWordCtr.uiDyPW4Out,0x02); //写入闭锁时间
                }
            }
        }
        return 0xFF;                                //返回密码不正确
...
uint8 Encode645Frame(S_COM* psCom, S_FRAMEINFO* sFrm)
{
    ...
    {
        if(RptRet() && (sFrm->uiID.lword != 0x04001501)
           && ((psCom->ucBuf[8]&0xf0) != 0xD0))  //否认帧无需再回主动上报状态字后续帧
        {
            psCom->ucBuf[8] |= 0x20;
